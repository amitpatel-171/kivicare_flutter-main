  ...
  init() async {
    ...
    if (isUpdate) {
      ...
      if (upcomingAppointment != null) {
        for (int i = 0;
            i < upcomingAppointment!.visit_type.validate().length;
            i++) {
          multiSelectStore.selectedService.add(ServiceData(
              id: upcomingAppointment!.visit_type![i].id,
              name: upcomingAppointment!.visit_type![i].service_name,
              service_id: upcomingAppointment!.visit_type![i].service_id));
        }
        servicesCont.text = multiSelectStore.selectedService.length.toString() +
            ' ' +
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          if (formKey.currentState!.validate()) {
            ...
            RAppointmentScreen2(updatedData: upcomingAppointment)
                .launch(context);
          }
        },
      ),
            ...
            child: AppTextField(
              ...
              onTap: () async {
                if (appointmentAppStore.mDoctorSelected == null &&
                  ...
                  if (!isUpdate) {
                    if (multiSelectStore.selectedService
                        .validate()
                        .isNotEmpty) {
                      multiSelectStore.selectedService.forEach((element) {
                        ids.add(element.id);
          ...
          isUpdate
              ? RDateComponent(
                  initialDate: DateTime.parse(
                      upcomingAppointment!.appointment_start_date.validate()),
                )
              : RDateComponent(),
          16.height,
          isUpdate
              ? AppointmentSlots(
                  ...
                  appointmentTime: DateFormat(TIME_WITH_SECONDS)
