...
class PatientEncounterListComponent extends StatefulWidget {
  ...
  PatientEncounterListComponent(
      {required this.data, this.tempDate, this.patientData});

  @override
  _PatientEncounterListComponentState createState() =>
      _PatientEncounterListComponentState();
}

class _PatientEncounterListComponentState
    extends State<PatientEncounterListComponent> {
  @override
  void initState() {
    super.initState();
    init();
            ...
            Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    Text(widget.data!.clinic_name.validate(),
                ...
                Row(
                  children: [
                    ...
                    4.width,
                    Text(widget.data!.doctor_name.validate(),
                            style: boldTextStyle(size: 16), maxLines: 2)
                        .flexible(),
                  ],
                ),
                8.height,
                Row(
                  children: [
                    Row(
                      ...
                      children: [
                        Text(locale.lblDescription + ': ',
                            style: secondaryTextStyle(size: 16)),
                        4.width,
                        Text(widget.data!.description.validate().trim(),
                    ...
                    Container(
                      ...
                      decoration: boxDecorationDefault(
                          borderRadius: radius(),
                          color: getEncounterStatusColor(widget.data!.status)
                              .withOpacity(0.2)),
                      child: Text(
                        "${getEncounterStatus(widget.data!.status)}"
                            .toUpperCase(),
                        style: boldTextStyle(
                            size: 10,
                            color:
                                getEncounterStatusColor(widget.data!.status)),
                      ),
                    )
                  ],
                ),
          ...
          SlidableAction(
            ...
            onPressed: (BuildContext context) async {
              showConfirmDialogCustom(
                ...
                title:
                    '${locale.lblDeleteRecordConfirmation} ${widget.data!.clinic_name.validate()}',
                onAccept: (p0) {
                  Map request = {
                  ...
                  deleteEncounterData(request).then((value) {
                    ...
                    toast(
                        " ${locale.lblAllRecordsFor} ${widget.data!.patient_name.validate()} ${locale.lblAreDeleted}");
                    init();
                    setState(() {});
                  }).catchError((e) {
                    appStore.setLoading(false);
