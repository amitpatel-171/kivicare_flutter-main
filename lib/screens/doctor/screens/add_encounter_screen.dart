...
class _AddEncounterScreenState extends State<AddEncounterScreen> {
  ...
  init() async {
    ...
    if (isUpdate) {
      ...
      encounterDateCont.text = patientEncounterData!.encounter_date!
  ...
  void updateEncounter() async {
    ...
    if (isDoctor()) {
      if (isProEnabled()) {
        request.putIfAbsent(
            "clinic_id", () => selectedClinic.clinic_id.validate());
      } else {
        request.putIfAbsent("clinic_id", () => getIntAsync(USER_CLINIC));
      }
    }
  ...
  void datePicker() async {
    ...
    if (dateTime != null) {
      encounterDateCont.text =
          dateTime.getFormattedDate(APPOINTMENT_DATE_FORMAT);
      current = dateTime;
      setState(() {});
    }
  }
      ...
      child: AnimatedScrollView(
        ...
        children: [
          ...
          AppTextField(
            ...
            decoration:
                inputDecoration(context: context, labelText: locale.lblDate),
          ),
          16.height,
          AppTextField(
