...
class _HolidayScreenState extends State<HolidayScreen> {
  ...
  Widget buildBodyWidget() {
    return FutureBuilder<HolidayModel>(
      ...
      builder: (_, snap) {
        if (snap.hasData) {
          if (snap.data!.holidayData.validate().isEmpty)
            return NoDataFoundWidget().center();
          ...
          return AnimatedScrollView(
            ...
            children: [
              Text(
                  locale.lblHolidays +
                      ' (${snap.data!.holidayData!.length.validate()})',
              ...
              AnimatedWrap(
                ...
                itemBuilder: (context, index) {
                  ...
                  int totalDays = (DateTime.parse(data.end_date!)
                          .difference(DateTime.parse(data.start_date!)))
                      .inDays;
                  int pendingDays = DateTime.parse(data.end_date!)
                      .difference(DateTime.now())
                      .inDays;
                  bool isPending =
                      (DateTime.parse(data.end_date!).isAfter(DateTime.now()));
                  ...
                  return Container(
                            ...
                            Column(
                              ...
                              children: [
                                if (data.module_type == 'doctor')
                                  if (listAppStore.doctorList.firstWhereOrNull(
                                          (element) =>
                                              element!.iD ==
                                              data.module_id.toInt()) !=
                                      null)
                                    Text(
                                ...
                                if (listAppStore.doctorList.firstWhereOrNull(
                                  ...
                                  10.height,
                                Text(
                                    '${data.start_date.validate().getFormattedDate(APPOINTMENT_DATE_FORMAT).validate()}',
                                    style: primaryTextStyle(size: 14)),
                                4.height,
                                Container(
                                    height: 1, width: 3, color: Colors.black),
                                4.height,
                                Text(
                                    '${data.end_date.validate().getFormattedDate(APPOINTMENT_DATE_FORMAT).validate()}',
                                    style: primaryTextStyle(size: 14)),
                                10.height,
                                Text(
                                        locale.lblAfter +
                                            ' ${pendingDays == 0 ? '1' : pendingDays} ' +
                                            locale.lblDays,
                                        style: boldTextStyle(size: 16))
                                    .visible(isPending),
                                Text(
                        ...
                        Positioned(
                          ...
                          child: Container(
                            ...
                            decoration: boxDecorationDefault(
                              ...
                              color: getHolidayStatusColor(isPending)
                                  .withOpacity(0.5),
                            ),
                          ),
                        ),
                    ...
                    () async {
                      await AddHolidayScreen(holidayData: data).launch(context,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBarWidget(locale.lblYourHolidays,
