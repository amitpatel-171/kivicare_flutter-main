  ...
  Widget googleMeet() {
    return SizedBox(
      ...
      child: Column(
        ...
        children: [
          if (isDisconnected)
            Text('${locale.lblYouAreConnectedWithTheGoogleCalender}.',
                style: boldTextStyle())
          else
            Text(
                '${locale.lblPleaseConnectWithYourGoogleAccountToGetAppointmentsInGoogleCalendarAutomatically}.',
                style: boldTextStyle()),
          CachedImageWidget(
            height: 150,
            width: 150,
          ...
          16.height,
          Text(userName.validate(value: ""), style: boldTextStyle(size: 24))
              .visible(userName.validate().isNotEmpty),
          16.height,
          Text(userEmail.validate(value: ''), style: secondaryTextStyle())
              .visible(userEmail.validate().isNotEmpty),
          16.height,
          if (!isDisconnected)
            AppButton(
            ...
            AppButton(
              ...
              onTap: () async {
                showConfirmDialogCustom(
                  context,
                  onAccept: (c) async {
                    disconnectMeet(request: {"doctor_id": appStore.userId})
                        .then((value) {
                      userName = "";
                      photoUrl = "";
                      userEmail = "";
        ...
        return Stack(
          ...
          children: [
            Column(
              children: [
                SettingItemWidget(
                  title: "${locale.lblGoogleMeet}" +
                      ' ${appStore.userTelemedOn.validate(value: false) ? locale.lblOn : locale.lblOff}',
                          ...
                          if (isZoomOn) {
                            if (appStore.telemedType == 'zoom') {
                              await showConfirmDialogCustom(
                                ...
                                title:
                                    "${locale.lblYouCanUseOneMeetingServiceAtTheTimeWeAreDisablingZoomService}.",
                                onAccept: (c) {
                                  disableZoom();
                                },
                              );
