...
class AddPrescriptionScreen extends StatefulWidget {
  ...
  AddPrescriptionScreen(
      {this.text, this.name, this.id, this.pID, this.prescriptionData});
    ...
    Widget body() {
      return FutureBuilder<PrescriptionModel>(
        ...
        builder: (context, snap) {
          if (snap.hasData) {
            if (pName.isEmpty) {
              log(snap.data!.prescriptionData
                  .validate()
                  .map((e) => e.name.capitalizeFirstLetter())
                  .toList()
                  .length);
              pName = snap.data!.prescriptionData
                  .validate()
                  .map((e) => e.name.capitalizeFirstLetter())
                  .toList();
            }
            if (pFrequency.isEmpty) {
              pFrequency = snap.data!.prescriptionData
                  .validate()
                  .map((e) => e.frequency.capitalizeFirstLetter())
                  .toList();
            }
            return Form(
              key: formKey,
              child: AnimatedScrollView(
                ...
                children: [
                  Text(locale.lblAddPrescription,
                  ...
                  Column(
                    children: [
                      AppTextField(
                        ...
                        validator: (s) {
                          if (s!.trim().isEmpty)
                            return locale.lblPrescriptionRequired;
                          return null;
                        },
                        ...
                        onTap: () async {
                          String? name = await showModalBottomSheet(
                            ...
                            builder: (context) {
                              return SelectionWithSearchWidget(
                                  searchList: pName,
                                  name: locale.lblPrescription);
                            },
                          );
                          if (name == null) {
                      ...
                      AppTextField(
                        ...
                        textFieldType: TextFieldType.OTHER,
                        decoration: inputDecoration(
                            context: context, labelText: locale.lblFrequency),
                        readOnly: true,
                        validator: (s) {
                          if (s!.trim().isEmpty)
                            return locale.lblPrescriptionFrequencyIsRequired;
                          return null;
                        },
                        onTap: () async {
                          String? name = await showModalBottomSheet(
                            ...
                            builder: (context) {
                              return SelectionWithSearchWidget(
                                  searchList: pFrequency,
                                  name: locale.lblFrequency);
                            },
                          );
                          if (name == null) {
