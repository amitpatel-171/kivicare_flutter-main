...
class _AppointmentFragmentState extends State<AppointmentFragment> {
  ...
  String endDate = DateTime(DateTime.now().year, DateTime.now().month + 8,
          DateTime.now().year + 5, Utils.lastDayOfMonth(DateTime.now()).day)
      .getFormattedDate(CONVERT_DATE);
  ...
  void showData(DateTime dateTime) async {
    ...
    await getAppointmentInCalender(
            todayDate: dateTime.getFormattedDate(CONVERT_DATE), page: 1)
        .then((value) {
          ...
          ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              return AppointmentWidget(
                      upcomingData: filterList[index], index: index)
                  .paddingSymmetric(vertical: 8);
            },
          ).visible(!appStore.isLoading, defaultWidget: LoaderWidget()),
          16.height,
          NoDataFoundWidget(
                  text: locale.lblNotAppointmentForThisDay, iconSize: 130)
              .visible(filterList.isEmpty && !appStore.isLoading)
              .center(),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () async {
          bool? res =
              await DoctorAddAppointmentStep1Screen(id: getIntAsync(USER_ID))
                  .launch(context);
