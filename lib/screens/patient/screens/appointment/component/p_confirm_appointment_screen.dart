// //  Url http://clinic.weborative.com/wp-json/kivicare/api/v2/appointment/save
...
class PConfirmAppointmentScreen extends StatefulWidget {
  ...
  @override
  _PConfirmAppointmentScreenState createState() =>
      _PConfirmAppointmentScreenState();
}
...
class _PConfirmAppointmentScreenState extends State<PConfirmAppointmentScreen> {
  ...
  DateTime appointmentDateTime = DateTime.parse(appointmentAppStore
  ...
  void saveAppointment() {
    ...
    Map<String, dynamic> req = {
      ...
      "appointment_start_date":
          "${appointmentAppStore.selectedAppointmentDate.getFormattedDate(CONVERT_DATE)}",
      "appointment_start_time":
          "${appointmentAppStore.mSelectedTime.validate()}",
    ...
    if (isProEnabled() != null) {
      req.putIfAbsent("clinic_id",
          () => "${appointmentAppStore.mClinicSelected?.clinic_id}");
    } else {
    ...
    if (appointmentAppStore.selectedService.isNotEmpty) {
      appointmentAppStore.selectedService.forEachIndexed((index, element) {
        req.putIfAbsent("visit_type[$index]",
            () => '${appointmentAppStore.selectedService[index]}');
      });
    }
    if (appointmentAppStore.reportList.isNotEmpty) {
      req.putIfAbsent(
          "attachment_count", () => "${appointmentAppStore.reportList.length}");

      appointmentAppStore.reportList.forEachIndexed((index, element) {
        req.putIfAbsent("appointment_report_$index",
    ...
    appointmentRequest.addAppointment(req).then((value) async {
      if (appStore.isBookedFromDashboard) {
        void test() {
          toast(
            value?.message.validate() ?? 'Appointment Booked Successfully',
          );
        ...
        //     ? await   WebViewPaymentScreen(
        //         checkoutUrl: value?.woocommerce_redirect.validate())
        //     .launch(context)
        //     .then((value) => toast(value?.message.validate() ??
        ...
        void test() {
          ...
          toast(value?.message.validate() ??
              locale.lblAppointmentBookedSuccessfully);
        ...
        value?.woocommerce_redirect != null
            ? WebViewPaymentScreen(
                checkoutUrl: value?.woocommerce_redirect.validate(),
              ).launch(context).then((value) => toast(
  ...
  Widget confirmedAppointment() {
    return Column(
      ...
      children: [
        ...
        30.height,
        Text(locale.lblAppointmentIsConfirmed,
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              mainAxisSize: MainAxisSize.min,
              children: [
                Text("${appointmentAppStore.mSelectedTime}",
                        style: boldTextStyle(size: 20),
                        textAlign: TextAlign.center)
                    .expand(),
                VerticalDivider(color: viewLineColor, thickness: 1)
                    .withHeight(20),
                16.width,
                Text("Dr. ${appointmentAppStore.mDoctorSelected?.display_name?.validate()}",
                        style: boldTextStyle(size: 20))
                    .expand(),
              ],
            ),
