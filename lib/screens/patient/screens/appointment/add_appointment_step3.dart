...
class AddAppointmentScreenStep3 extends StatefulWidget {
  ...
  @override
  _AddAppointmentScreenStep3State createState() =>
      _AddAppointmentScreenStep3State();
}
...
class _AddAppointmentScreenStep3State extends State<AddAppointmentScreenStep3> {
  ...
  init() async {
    ...
    if (isUpdate) {
      ...
      if (upcomingAppointment!.appointment_report != null ||
          upcomingAppointment!.appointment_report.validate().isNotEmpty) {
        appointmentAppStore.addReportListString(
            data: upcomingAppointment!.appointment_report!);
      }

      log(appointmentAppStore.reportListString.length);

      await getDoctor(
              clinicId: upcomingAppointment!.clinic_id.validate().toInt())
          .then((value) {
        //edited here 11-10-2023
        appointmentAppStore.setSelectedDoctor(listAppStore.doctorList
            .firstWhereOrNull(
                (element) => element!.iD == getIntAsync(USER_ID)));
      });

      await getClinic().then((value) {
        appointmentAppStore.setSelectedClinic(value.firstWhereOrNull(
          (element) =>
              element.clinic_id.validate() ==
              upcomingAppointment!.clinic_id.validate(),
        ));
      ...
      children: [
        AppTextField(
          ...
          decoration: inputDecoration(
              context: context, labelText: locale.lblSelectServices),
          readOnly: true,
          onTap: () async {
            if (!isUpdate) {
            ...
            bool? res = await MultiSelectWidget(selectedServicesId: ids)
                .launch(context);
            if (res ?? false) {
              List<int> temp = [];
              multiSelectStore.selectedService.forEach((element) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
              ...
              children: [
                ...
                PDateComponent(
                    initialDate: isUpdate
                        ? DateTime.parse(upcomingAppointment!
                ...
                AppointmentSlots(
                  doctorId:
                      isUpdate ? upcomingAppointment!.doctor_id.toInt() : null,
                  appointmentTime: isUpdate
                      ? DateFormat(TIME_WITH_SECONDS)
                          .parse(upcomingAppointment!.appointment_start_time!)
                          .getFormattedDate(FORMAT_24_HOUR)
                      : null,
                ),
                AbsorbPointer(
                  absorbing: isUpdate,
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text(locale.lblBook, style: boldTextStyle(color: white))
        ...
        onPressed: () async {
          if (formKey.currentState!.validate()) {
            ...
            await showInDialog(
              ...
              builder: (p0) {
                return isUpdate
                    ? PConfirmAppointmentScreen(
                        appointmentId: widget.data?.id.toInt())
                    : PConfirmAppointmentScreen();
              },
            );
          }
