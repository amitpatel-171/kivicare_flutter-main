...
class PatientEncounterDashboardScreen extends StatefulWidget {
  ...
  @override
  _PatientEncounterDashboardScreenState createState() =>
...
class _PatientEncounterDashboardScreenState
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text(locale.lblEncounterDate + ':',
                      style: secondaryTextStyle(size: 16)),
                  8.height,
                  Text(locale.lblClinicName + ':',
                      style: secondaryTextStyle(size: 16)),
                  8.height,
                  Text(locale.lblDoctorName + ':',
                      style: secondaryTextStyle(size: 16)),
                  8.height,
              ...
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('${data.patient_name.validate()}',
                      style: boldTextStyle(size: 16)),
                  8.height,
                  Text('${data.patient_email.validate()}',
                      style: boldTextStyle(size: 16)),
                  8.height,
                  Text(
                      '${data.encounter_date.validate()}'
                          .capitalizeFirstLetter(),
                      style: boldTextStyle(size: 16)),
                  8.height,
                  Text('${data.clinic_name.validate()}',
                      style: boldTextStyle(size: 16)),
                  8.height,
                  Text('${data.doctor_name.validate()}',
                      style: boldTextStyle(size: 16)),
                  8.height,
                  if (data.description.validate().isNotEmpty)
                    Text(
          ...
          Align(
            ...
            child: Container(
              ...
              decoration: boxDecorationDefault(
                  color: getEncounterStatusColor(data.status).withOpacity(0.2),
                  borderRadius: BorderRadius.circular(defaultRadius)),
              child: Text(
                "${getEncounterStatus(data.status.validate())}".toUpperCase(),
                style: boldTextStyle(
                    size: 12, color: getEncounterStatusColor(data.status)),
              ).center(),
            ),
          ),
        ],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBarWidget(locale.lblEncounterDashboard,
          ...
          if (snap.hasData) {
            ...
            if (dataWidgets.isEmpty) {
              if (getBoolAsync(USER_PRO_ENABLED)) {
                snap.data!.enocunter_modules.validate().forEach((element) {
                  if (element.status.toInt() == 1) {
                    if (element.name == "problem") {
                      dataWidgets.add(ProblemListWidget(
                          medicalHistory: snap.data!.problem,
                          encounterType: PROBLEM));
                      dataWidgets.add(Divider(color: viewLineColor));
                    } else if (element.name == "observation") {
                      dataWidgets.add(ProblemListWidget(
                          medicalHistory: snap.data!.observation,
                          encounterType: OBSERVATION));
                      dataWidgets.add(Divider(color: viewLineColor));
                    } else if (element.name == "note") {
                      dataWidgets.add(ProblemListWidget(
                          medicalHistory: snap.data!.note,
                          encounterType: NOTE));
                      dataWidgets.add(Divider(color: viewLineColor));
                    }
                  }
                ...
                snap.data!.prescription_module.validate().forEach((element) {
                  if (element.status.toInt() == 1) {
                    if (element.name == "prescription") {
                      dataWidgets.add(PrescriptionWidget(
                ...
                dataWidgets.add(ProblemListWidget(
                    medicalHistory: snap.data!.note, encounterType: NOTE));
                dataWidgets.add(Divider(color: viewLineColor));

                dataWidgets.add(
                    PrescriptionWidget(prescription: snap.data!.prescription));
              }
            }
                ...
                children: [
                  ...
                  isProEnabled()
                      ? Container(
                          ...
                          padding: EdgeInsets.all(16),
                          child: Text(locale.lblViewAllReports,
                                  style: boldTextStyle())
                              .center()
                              .onTap(() {
                            PatientReportScreen(
                                    patientId: snap.data!.patient_id.toInt())
                                .launch(context);
                          }),
                        )
                      : Offstage(),
      ...
      //     FloatingActionButton(child: Text("Pay Bill"), onPressed: () {}),
    );
  }
}
