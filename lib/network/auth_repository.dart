...
Future<LoginResponseModel> login(Map req) async {
  LoginResponseModel value = LoginResponseModel.fromJson(await (handleResponse(
  ...
  if (value.clinic.validate().isNotEmpty) {
    appStore.setCurrency(value.clinic!.first.extra!.currency_prefix.validate(),
        initiliaze: true);
  ...
  appStore.setUserDisplayName(value.user_display_name.validate(),
      initiliaze: true);
  appStore.setUserMobileNumber(value.mobile_number.validate(),
      initiliaze: true);
  ...
  appStore.setUserEnableGoogleCal(value.is_enable_google_cal.validate(),
      initiliaze: true);
  appStore.setUserDoctorGoogleCal(value.is_enable_doctor_gcal.validate(),
      initiliaze: true);
...
Future<bool> updateProfile(BuildContext context, Map data,
    {File? file, String? toastMessage}) async {
  ...
  if (response.statusCode.isSuccessful()) {
    LoginResponseModel data =
        LoginResponseModel.fromJson(jsonDecode(response.body)['data']);
...
Future<bool> updateReceptionistDoctor(Map data,
    {File? file, String? toastMessage}) async {
  var multiPartRequest =
      await getMultiPartRequest('kivicare/api/v1/doctor/add-doctor');
...
Future<bool> updatePatientProfile(Map data,
    {File? file, String? toastMessage}) async {
  var multiPartRequest =
      await getMultiPartRequest('kivicare/api/v1/user/profile-update');
