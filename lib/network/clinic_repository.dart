...
Future<ClinicListModel> getClinicList({int? page}) async {
  return ClinicListModel.fromJson(await (handleResponse(await buildHttpResponse(
      'kivicare/api/v1/clinic/get-list?page=${page != null ? page : ''}'))));
}

Future<List<Clinic>> getClinicListNew(
    {required int page,
    required List<Clinic> appointmentList,
  ...
  ClinicListModel res = ClinicListModel.fromJson(await (handleResponse(
      await buildHttpResponse(getEndPoint(
          endPoint: 'kivicare/api/v1/clinic/get-list', page: page)))));
