...
Future<NewsModel> getNewsList() async {
  return NewsModel.fromJson(await (handleResponse(
      await buildHttpResponse('kivicare/api/v1/news/get-news-list'))));
}

Future<PatientListModel> getPatientList({int? page}) async {
  if (page == null) {
    return PatientListModel.fromJson(await (handleResponse(
        await buildHttpResponse('kivicare/api/v1/patient/get-list?limit=-1'))));
  } else {
    return PatientListModel.fromJson(await (handleResponse(
        await buildHttpResponse(
            'kivicare/api/v1/patient/get-list?limit=$PER_PAGE&page=$page'))));
  }
}

Future<List<PatientData>> getPatientListNew(
    {required int page,
    required List<PatientData> patientList,
  ...
  PatientListModel res = PatientListModel.fromJson(await handleResponse(
      await buildHttpResponse(getEndPoint(
          endPoint: 'kivicare/api/v1/patient/get-list', page: page))));
