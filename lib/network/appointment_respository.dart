...
Future<PatientEncounterModel> getPatientAppointmentList(
    {String? status, int? patientId}) async {
  PatientEncounterModel value = PatientEncounterModel.fromJson(
...
Future<List<UpcomingAppointment>> getPatientAppointmentListNew(int? patientId,
    {String? status,
    page,
    required List<UpcomingAppointment> appointmentList,
    Function(bool)? lastPageCallback,
    Function(int)? getTotalPatient}) async {
  ...
  PatientEncounterModel res = PatientEncounterModel.fromJson(
      await (handleResponse(await buildHttpResponse(
    getEndPoint(
...
Future<AppointmentModel> getAppointmentData(
    {bool isPast = false,
    String? todayDate,
    String? startDate,
    String? endDate,
  ...
  if (todayDate == null) {
    value = AppointmentModel.fromJson(await (handleResponse(await buildHttpResponse(
        'kivicare/api/v1/appointment/get-appointment?page=1&limit=10&start=$startDate&end=$endDate'))));
  } else {
    if (isPast) {
      value = AppointmentModel.fromJson(await (handleResponse(
          await buildHttpResponse(
              'kivicare/api/v1/appointment/get-appointment?status=$status&page=$page&limit=$PER_PAGE'))));
    } else {
      value = AppointmentModel.fromJson(await (handleResponse(
          await buildHttpResponse(
              'kivicare/api/v1/appointment/get-appointment?status=$status&page=$page&limit=$PER_PAGE&date=$todayDate'))));
    }
  }
  return value;
}

Future<AppointmentModel> getAppointmentInCalender(
    {String? todayDate, int? page}) async {
  AppointmentModel value;

  value = AppointmentModel.fromJson(await (handleResponse(await buildHttpResponse(
      'kivicare/api/v1/appointment/get-appointment?&page=$page&limit=$PER_PAGE&date=$todayDate'))));
  return value;
}

Future<List<List<AppointmentSlotModel>>> getAppointmentList(
    {String? appointmentDate, int? id, int? clinicId}) async {
  Iterable it = await (handleResponse(await buildHttpResponse(
      'kivicare/api/v1/doctor/appointment-time-slot?clinic_id=$clinicId&date=$appointmentDate&doctor_id=${id != null ? id : ''}&appointment_id=')));
...
class AppointmentRequest {
  Future<ConfirmAppointmentResponseModel?> addAppointment(
    ...
    multiPartRequest.fields['appointment_start_date'] =
        data['appointment_start_date'];
    multiPartRequest.fields['appointment_start_time'] =
    ...
    if (appointmentAppStore.selectedService.isNotEmpty) {
      appointmentAppStore.selectedService.forEachIndexed((index, element) {
        multiPartRequest.fields["visit_type[$element]"] =
            data['visit_type[$element]'];
      });
    }
    ...
    if (appointmentAppStore.reportList.isNotEmpty) {
      ...
      await Future.forEach<PlatformFile>(appointmentAppStore.reportList,
          (element) async {
        multiPartRequest.files.add(await MultipartFile.fromPath(
            'appointment_report_${appointmentAppStore.reportList.indexOf(element)}',
            File(element.path.validate()).path));
      });
    }

    multiPartRequest.headers.addAll(buildHeaderTokens());

    return await Response.fromStream(await multiPartRequest.send())
        .then((value) {
