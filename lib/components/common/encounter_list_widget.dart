    ...
    if (formKey.currentState!.validate()) {
      ...
      await saveMedicalHistoryData(request).then((value) {
        ...
        getMedicalHistoryResponse(
            widget.id.validate(), widget.encounterType!.toLowerCase());
        toast(
            "${widget.encounterType.capitalizeFirstLetter() + locale.lblAdded}");
      }).catchError((e) {
        toast(e.toString());
      });
      ...
      child: Stack(
        children: [
          FutureBuilder<MedicalHistoryModel>(
            future: _memorizer.runOnce(() => getMedicalHistoryResponse(
                widget.id.validate(), widget.encounterType!.toLowerCase())),
              ...
              if (snap.hasData) {
                ...
                return SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                          "${widget.encounterType.capitalizeFirstLetter()}s (${encounterTypeList!.length})",
                          style: boldTextStyle()),
                      16.height,
                      ListView.separated(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          ...
                          if (encounterTypeList != null &&
                              index >= 0 &&
                              index < encounterTypeList!.length) {
                            data = encounterTypeList?[index];
                          } else {
                            throw Exception(e);
                          }
            ...
            builder: (context) {
              return NoDataFoundWidget(
                ...
                iconSize: 130,
                  ...
                  appStore.isLoading || encounterTypeList.validate().isEmpty);
            },
          ),
                      ...
                      child: AppTextField(
                        ...
                        errorThisFieldRequired:
                            "${widget.encounterType.capitalizeFirstLetter()} " +
                                locale.lblFieldIsRequired,
                        decoration: inputDecoration(
                            context: context,
                            labelText:
                                locale.lblEnter + ' ${widget.encounterType}'),
