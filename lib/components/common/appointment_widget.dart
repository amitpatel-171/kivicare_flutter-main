...
class _AppointmentWidgetState extends State<AppointmentWidget> {
      ...
      onAccept: (ctx) {
        if (widget.upcomingData!.status.toInt() == 1) {
          ...
          if (getStringAsync(USER_DATA).isNotEmpty) {
            finish(context);
            push(EncounterDashboardScreen(
                id: widget.upcomingData!.encounter_id));
          }
        }
      },
    );
  ...
  void deleteAppointmentValue(BuildContext context) async {
    bool? res = await showConfirmDialog(context, locale.lblAreDeleteAppointment,
  ...
  bool get isCheckIn {
    ...
    if (isPatient()) {
      ...
      if (widget.upcomingData!.status.toInt().getStatus() != CancelledStatus &&
          widget.upcomingData!.status.toInt().getStatus() != CheckOutStatus) {
        mIsCheckIn =
            DateTime.parse(widget.upcomingData!.appointment_start_date!)
                    .difference(DateTime.parse(
                        DateFormat(CONVERT_DATE).format(DateTime.now())))
                    .inDays ==
                0;
      }
    }
  ...
  bool get isEncounterDashboard {
    return widget.upcomingData!.status.toInt().getStatus() == CheckInStatus ||
        widget.upcomingData!.status.toInt().getStatus() == CheckOutStatus;
  }
  ...
  bool get isEdit {
    return widget.upcomingData!.status.toInt().getStatus() != CheckOutStatus &&
        ...
        widget.upcomingData!.status.toInt().getStatus() != CheckInStatus &&
        DateTime.parse(widget.upcomingData!.appointment_start_date.validate())
      ...
      endActionPane: ActionPane(
        ...
        children: [
          SlidableAction(
            onPressed: (BuildContext context) {
              if (isPatient()) {
                ...
                RAppointment1Screen(
                        id: getIntAsync(USER_ID), data: widget.upcomingData)
                    .launch(
                  context,
                  pageRouteAnimation: PageRouteAnimation.Slide,
                );
              } else {
                DoctorAddAppointmentStep1Screen(
                        id: getIntAsync(USER_ID),
                        appointmentData: widget.upcomingData)
                    .launch(
                  context,
                  pageRouteAnimation: PageRouteAnimation.Slide,
                );
          ...
          SlidableAction(
            ...
            borderRadius:
                radiusOnly(topRight: defaultRadius, bottomRight: defaultRadius),
        ...
        children: [
                          ...
                          children: [
                            if (isReceptionist())
                              CommonRowWidget(
                                ...
                                value:
                                    widget.upcomingData!.doctor_name.validate(),
                                isMarquee: true,
                              ),
                            CommonRowWidget(
                              title: locale.lblService,
                              value:
                                  '${widget.upcomingData!.visit_type.validate().map((e) => e.service_name.validate()).join(" , ")}',
                            ),
                            CommonRowWidget(
                              title: locale.lblDate,
                              value: widget.upcomingData!.appointment_start_date
                                  .validate()
                                  .getFormattedDate(APPOINTMENT_DATE_FORMAT),
                            ),
                            CommonRowWidget(
                                title: locale.lblTime,
                                value:
                                    '${DateFormat(TIME_WITH_SECONDS).parse(widget.upcomingData!.appointment_start_time!).getFormattedDate(FORMAT_24_HOUR)} - ${DateFormat(TIME_WITH_SECONDS).parse(widget.upcomingData!.appointment_end_time!).getFormattedDate(FORMAT_24_HOUR)}'),
                            CommonRowWidget(
                              title: locale.lblDesc,
                              value: widget.upcomingData!.description
                                      .validate()
                                      .trim()
                                      .isNotEmpty
                                  ? widget.upcomingData!.description.validate()
                                  : 'NA',
                            ),
                            CommonRowWidget(
                              title: locale.lblPrice,
                              value:
                                  '${appStore.currency.validate()}${widget.upcomingData!.all_service_charges}',
                      ...
                      children: [
                        Flexible(
                          child: AppButton(
                            ...
                            child: FittedBox(
                              child: FittedBox(
                                child: Text(locale.lblViewDetails,
                                    style:
                                  ...
                                  children: [
                                    ...
                                    Positioned(
                                      ...
                                      child: Container(
                                        ...
                                        child: Text(
                                          getStatus(widget.upcomingData!.status
                                              .validate()),
                                          style: boldTextStyle(
                                              size: 12, color: white),
                                        ),
                        ...
                        Flexible(
                          child: AppButton(
                            ...
                            child: FittedBox(
                                child: Text(locale.lblEncounter,
                                    style:
                                        boldTextStyle(color: white, size: 12))),
                        ...
                        Flexible(
                          child: AppButton(
                            ...
                            child: FittedBox(
                              child: Text(locale.lblCheckIn,
                                      style:
                                          boldTextStyle(color: white, size: 12))
                                  .visible(
                                widget.upcomingData!.status
                                        .toInt()
                                        .getStatus() !=
                                    CheckInStatus,
                                defaultWidget: FittedBox(
                                  child: Text(locale.lblCheckOut,
                                      style: boldTextStyle(
                                          color: white, size: 12)),
                                ),
                              ),
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: Text(getStatus(widget.upcomingData!.status.validate()),
